...
describe 'Deployment' do
  describe ec2(EC2Helper.get_ec2_instance_id_from_tag_name(vars.get('meta_name'), vars.get('aws_region'))) do
    it { should exist }
    it { should be_running }

    it { should have_tag('Name').value(vars.get('meta_name')) }
    ...
    it { should have_security_group(vars.get('meta_name') + '-sg') }
